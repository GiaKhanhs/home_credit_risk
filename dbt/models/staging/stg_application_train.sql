WITH stg_application_train__source AS (
    SELECT *
    FROM {{ source('raw','application_train') }}
)
    , stg_application_train__redefined AS (
        SELECT
            -- =========================
            -- Primary key / target
            -- =========================
            CAST(sk_id_curr AS INTEGER)                            AS sk_id_curr,
            CAST(target AS SMALLINT)                               AS target,
        ...
        FROM stg_application_train__source 
        )

SELECT *
FROM stg_application_train__redefined